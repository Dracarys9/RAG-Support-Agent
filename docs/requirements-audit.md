...
|---|---|---|---|
| Use RAG over `knowledge-base/` | Partial | `src/rag_support_agent/knowledge_base.py`, `src/rag_support_agent/support_agent.py` | The project loads, splits, searches, and cites knowledge-base sections. It does not use a generative model; it uses deterministic answers from retrieved text. This is documented as `Model: None` and `Embeddings: None` in `README.md`. |
...
| Retrieve relevant passages, not the whole corpus | Complete | `knowledge_base.py: search_knowledge_base`, `support_agent.py: _answer_from_knowledge` | Search returns a limited number of sections and the answer uses selected sections. The visible evaluator checks source selection. |
...
| Do not send the entire orders file to the answer layer | Complete for this local design | `orders.py: _safe_order`, `support_agent.py: _answer_order` | Only selected safe fields are returned to the support program. There is no model prompt in this implementation. |
...
| Use company content over general knowledge | Complete for supported questions | Answers come from selected company sections or safe order data | No external knowledge source is used. Unsupported questions abstain. |
...
| Retrieved passages, metadata, and scores | Complete | `SupportResponse.retrieved_passages`, `DebugTrace.retrieved_passages` | Debug JSON now includes passage text, filename, heading, metadata, and score. |
| Tool calls and sanitized tool results | Complete | `SupportResponse.sanitized_tool_result`, `DebugTrace.sanitized_tool_result` | Debug JSON includes safe tool output without private fields. |
...
| Model, embeddings, framework, storage choices | Complete | `README.md: Technology choices` | Explicitly states deterministic local implementation, no model, no embeddings, plain Python, and in-memory file storage. |
...
The core support agent is working strongly against the supplied cases: **33 regular tests pass, 15/15 visible cases pass, and 5/5 original cases pass**. The debug trace now includes retrieved text, metadata, scores, sanitized tool results, and fallback reasons.
